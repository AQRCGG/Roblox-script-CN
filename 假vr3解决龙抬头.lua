...
task.spawn(function()
    ...
    pcall(function()
        if UIS.TouchEnabled then
            ...
            -- 5. 右半屏滑动控制视角（单指绑定优化，防双指干扰，不影响按钮）
            local touchLook = false
            local activePointerId = nil
            local lastTouch = Vector2.zero
            local touchSens = 0.0025
            UIS.TouchStarted:Connect(function(i)
                -- 仅右半屏按下、且当前无正在控制视角的手指时才激活
                if not touchLook and i.Position.X > workspace.CurrentCamera.ViewportSize.X * 0.5 then
                    local vpY = workspace.CurrentCamera.ViewportSize.Y
                    if i.Position.Y < vpY * 0.85 and i.Position.Y > vpY * 0.15 then
                        touchLook = true
                        activePointerId = i.PointerId -- 绑定当前手指的唯一ID
                        lastTouch = i.Position
                    end
                end
            end)
            UIS.TouchMoved:Connect(function(i)
                -- 只有绑定的那根手指移动才会转视角，其他手指完全忽略
                if touchLook and i.PointerId == activePointerId then
            ...
            UIS.TouchEnded:Connect(function(i)
                -- 只有绑定的手指松开才退出视角控制
                if i.PointerId == activePointerId then
                    touchLook = false
                    activePointerId = nil
                end
            end)
